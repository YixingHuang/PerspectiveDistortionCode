...
if __name__ == '__main__':
    ...
    optVal.num_threads = 0  # test code only supports num_threads = 0
    optVal.batch_size = 30  # test code only supports batch_size = 1
    ...
    optVal.display_id = -1
    datasetVal = create_dataset(optVal)
...
for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        visualizer.print_current_losses_train2(running_loss)

        model.isTrain = False
        with torch.no_grad():
            for i, data in enumerate(datasetVal):  # inner loop within one epoch
                model.set_input(data)         # unpack data from dataset and apply preprocessing
                model.forward()   # calculate loss functions, get gradients, update network weights
                model.cal_l1_loss()
                losses_val = model.get_current_losses()
                visualizer.print_current_losses_val(losses_val)
        model.isTrain = True
